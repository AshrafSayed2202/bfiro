...
const ProductsPage = ({ title, subTitle, formatsList, apiEndpoint }) => {
    const [isOpen, setIsOpen] = useState(false);
    const [scrolled, setScrolled] = useState(false);
    const [ticks, setTicks] = useState([]); // Array of scroll positions
    const [currentTick, setCurrentTick] = useState(0); // Current tick index
    const allLabels = ["Mobile", "Web", "Travel", "Food", "Social", "Ecommerce", "Health", "Finance", "Ecommerce1", "Health1", "Finance1"];
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Simulate API call to apiEndpoint
                const mockResponse = Array.from({ length: 200 }, (_, i) => {
                    const selectedLabels = [...new Set(Array.from({ length: Math.floor(Math.random() * 3) + 1 }, () => allLabels[Math.floor(Math.random() * allLabels.length)]))];
                    ...
                    return {
                        ...
                        title: `${title} Variant ${i + 1} - Car Parking and Charging App`,
                        type: title,
    ...
    const checkArrows = () => {
        if (labelsRef.current) {
            ...
            // Calculate ticks: Always start with 0, then 200px steps, last tick is remainder
            const tickSize = 200;
            ...
            // Find closest tick to current scroll position
            ...
            // Snap to last tick if at or beyond maxScrollLeft (within tolerance)
            ...
            setShowLeft(scrollLeft > tolerance); // Show left if not at start
            setShowRight(scrollLeft < maxScrollLeft - tolerance); // Show right if not at end
        }
    };
    ...
    const scrollLeft = () => {
        if (labelsRef.current && currentTick > 0 && ticks.length > 0) {
            ...
            setTimeout(() => {
                if (labelsRef.current) {
                    // Force exact position to prevent drift
                    labelsRef.current.scrollTo({ left: ticks[newTick], behavior: 'instant' });
                    checkArrows(); // Update arrows after animation
                }
            }, 300);
        }
    };
    ...
    useEffect(() => {
        ...
        const handleResize = () => {
            // On resize, recalculate and snap to closest tick
            checkArrows();
            if (ticks.length > 0) {
                ...
                // Snap to last tick if at or beyond maxScrollLeft
                    ...
                    <div className={`w-screen mb-8 p-3 sticky top-[90px] md:top-[100px] z-10 ${scrolled ? "bg-[#121212a6] filter-blur-4" : "bg-transparent"}`}>
                        <div className="products-labels content-contain flex flex-row gap-4 justify-between items-center">
                            <div className="relative flex gap-2 text-[18px] font-[600] overflow-x-auto sm:overflow-hidden scrollbar-hide" >
                                ...
                                <div className={`relative flex gap-2 text-[18px] font-[600] overflow-x-auto sm:overflow-hidden scrollbar-hide ${showLeft ? 'filters-scroll-left' : ''} ${showRight ? 'filters-scroll-right' : ''} ${showLeft && showRight ? 'filters-scroll-both' : ''}`} ref={labelsRef}>
                                    <button
                                        onClick={clearLabels}
                        ...
                        <AnimatePresence>
                            {displayed.map((item, index) => (
                                ...
                                >
                                    <ItemCard
                                        item={item}
                                        className={`scale-100 !duration-500 !transition-all hover:scale-[1.02] delay-${index % 4 * 100}`}
                                    />
                                </motion.div>
                            ))}
                        </AnimatePresence>
